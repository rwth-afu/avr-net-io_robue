/*----------------------------------------------------------------------------
 ...
 Modifiziert für AVR-NET-IO:	RoBue + cni
 Version:			1.5.1
 Datum:				05.02.2009
...
// VA24-28	-> Reserve
// VA-Ende	-> Counter
unsigned int var_array[MAX_VAR_ARRAY] = {10,50,30,0,0,0,0,0,120,0,15,15,15,15,15,15,15,15,750,750,12,0,12,0,0,0,0,0,0,0};
...
// *************
	...
	#if USE_OW
	// RoBue:
	// Zurücksetzen der Min/Max-Werte um 00:00:00 Uhr einschalten
	...
	if ( ss%10 == 1 ) {
	// Schalten wieder ermöglichen
		schalten = 1;
	//Zurücksetzen der möglichen Funkimpulse nach 1s
	//	PORTB &= ~(1 << PB0); // aus
	//	PORTB &= ~(1 << PB3); // aus
	}
	...
	// ------------------------------------------------------
	// Funkschalter PORTB0/B2:
	// Über Analogwert:
	//if ( var_array[6] > var_array[18] ) {
	//	if ( Funk_1 == 0 ) {
	//		PORTB |= (1 << PB0); // Einschalten ein
	//		Funk_1 = 1;
	//	}			
	//}
	//else {	
	//	if (Funk_1 == 1) {
	//		PORTB |= (1 << PB3); // Ausschalten ein
	//		Funk_1 = 0;
	//	}
	//}
	// -------------------------------------------------------
